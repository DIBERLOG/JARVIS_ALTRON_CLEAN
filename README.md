...
### Дополнительные идеи и требования владельца

Ниже — расширение плана, сформированное из списка идей. Пункты разделены по смыслу, чтобы их можно было реализовывать поэтапно и тестировать по одному.

#### Погода и ежедневные рекомендации

- Показывать расширенный прогноз на несколько дней и неделю в «Центре».
- Давать рекомендации по одежде на основе температуры, осадков, ветра и времени суток: например, предупредить о дожде или предложить взять куртку.
- Связывать погоду с календарём и привычками: напоминать о тренировке с учётом прогноза.

#### Голос, чат и диалог

- Добавить кнопку голосового ввода в текстовый чат: голос → распознавание речи → текст в поле сообщения → ответ модели → озвучивание.
- Улучшить преобразование речи в текст и текста в речь; сохранить Silero как основной вариант после устранения задержки.
- В голосовом разговоре распознавать обычные команды из общего списка, а не только вопросы к модели. Пример: во время диалога «открой Discord» должен запускать Discord, а не уходить в чат.
- Сохранять историю запросов и ответов разговорного режима отдельно от обычных команд, с возможностью просмотра и очистки.

#### Заметки, память и персонализация

- Создавать заметки голосом и добавлять текст к уже существующей заметке: «Джарвис, допиши к заметке “Тренировки” …».
- Постепенно запоминать пользовательские формулировки команд, предпочтения и голосовой профиль только локально и с понятным управлением этими данными.
- Добавить экран памяти: что JARVIS сохранил, как это исправить и как удалить отдельную запись или всё сразу.

#### Игровой режим

- Расширить команду «игровой режим»: открывать Steam, Discord и Яндекс Музыку.
- Сделать настройку состава игрового режима: пользователь выбирает запускаемые приложения и плейлист / сайт.

#### Интернет и новости

- Добавить новостную сводку мира по голосовой команде и в разделе «Сегодня».
- Показывать состояние интернет-подключения и объяснять, какие функции требуют сети: погода, новости, веб-поиск, DeepSeek.
- Для новостей и поиска показывать источники, время публикации и возможность открыть материал в браузере.

#### Android

- Спроектировать Android-версию после стабилизации Windows-версии и формата локальных данных.
- На первом этапе Android должен уметь показывать «Центр», заметки, календарь, привычки и историю диалогов; управление Windows-командами возможно добавить позже через защищённое локальное соединение.
